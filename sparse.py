...
class SparseExperiment(DenseExperiment):
    ...
    _DEFAULT_ACT_FNS = {
        'encoding': 'relu',
        # binarize_code means use BinaryActivation layer after encoding (use 'relu' for the encoding w/this feature)
        'binarize_code': True,
        'internal': 'relu',
    }

    def __init__(self, enc_layers=(64,), n_epochs=25, act_fns=None, reg_lambda=0.5):
        activation_functions = self._DEFAULT_ACT_FNS.copy() if act_fns is None else act_fns
        super().__init__(enc_layers=enc_layers, n_epochs=n_epochs, act_fns=activation_functions)
